  ...
  <v-container fluid class="py-2 px-0" style="margin-top: 80px">
    ...
    <v-card class="pe-8" tile elevation="0">
      ...
      >
        ...
        <template v-slot:item.file_gambar_struktur_organisasi="{ item }">
          <img
            :src="`${baseUrl}/storage/${item.file_gambar_struktur_organisasi}`"
    ...
    <v-card class="py-3" rounded="xl">
      ...
      <v-card-text class="py-3" v-if="dialogAction !== 'hapus'">
        <v-form @submit.prevent>
          <v-row>
            <v-col cols="12">
              <v-file-input
                v-model="form.file_gambar_struktur_organisasi"
                label="Gambar"
                variant="outlined"
                rounded="xl"
                accept=".jpg, .jpeg, .png"
      ...
      <v-card-actions class="text-center" align="center" justify="center">
        ...
        <v-btn
          ...
          size="small"
          @click="suntingStrukturOrganisasi()"
          >Ya</v-btn
        >
      </v-card-actions>
    </v-card>
...
const openDialog = (action, item, handler) => {
  form.value = { ...item };
  form.value.file_gambar_struktur_organisasi = null;
...
const suntingStrukturOrganisasi = () => {
